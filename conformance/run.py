...
def generate() -> None:
    ...
    with Store.open(STORE) as s:
        ops = Ops(s)
        cases = [{"op": c["op"], "args": c["args"], "expect": run_call(ops, c["op"], c["args"])} for c in CALLS]
...
def check() -> int:
    ...
    print(f"{len(data['cases']) - failures}/{len(data['cases'])} conformance cases pass")
